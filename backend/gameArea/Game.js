class Game {
    constructor(gameMode,playerOneKey,playerTwoKey) {
        this.numRows = 6
        this.numCells = 7
        this.maxDepth = 5
        this.gameMode = gameMode
        this.activePlayer = playerOneKey
        this.isRunning = true
    ...
    resetGame(mode) {
        ...
        this.gameBoard = Array(this.numRows).fill("").map(() => Array(this.numCells).fill(""))
        this.activePlayer = this.playerOneKey
        this.isRunning = true
        this.winner = undefined
    }
    ...
    nextMove(cell) {
        ...
        if(cell >= 0 && cell < this.numCells && this.isRunning) {
            if(this.gameMode === 1) {
                ...
                if(this.gameMode === 1 && this.isRunning) {
                    this.aiMakeMove()
                    if(this.checkWinner()) {
                        this.isRunning = false
                        this.winner = "red"  
                ...
                const nextPlayer = this.activePlayer === this.playerOneKey ? "blue" : "red"
                this.makeMove(cell,nextPlayer)
                if(this.checkWinner()) {
                    this.isRunning = false
                    this.winner = nextPlayer  
                }
                this.activePlayer = this.activePlayer === this.playerOneKey ? this.playerTwoKey : this.playerOneKey 
            }
